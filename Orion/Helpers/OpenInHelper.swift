...
class DownloadHelper: NSObject, OpenInHelper {
    ...
    fileprivate static let appIDToken = UUID().uuidString

    static func requestDownload(url: URL, webView: WKWebView?) {
        let safeUrl = url.absoluteString.replacingOccurrences(of: "'", with: "%27")
        webView?.evaluateJavascriptInDefaultContentWorld("window.__firefox__.download('\(safeUrl)', '\(appIDToken)')", { (result, error) in
            print("Result is = \(result), error: \(error)")
        })
